...
from sqlalchemy import (
    String, Integer, Float, DateTime, Date, Text, JSON, ForeignKey,
...
class Project(Base):
    ...
    status: Mapped[str] = mapped_column(String(50), default="active")  # active, paused, archived, completed
    ...
    extra_data: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)  # Extra metadata like commands from CLAUDE.md
...
class Goal(Base):
    ...
    parent_goal_id: Mapped[Optional[int]] = mapped_column(ForeignKey("goals.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    parent_goal: Mapped[Optional["Goal"]] = relationship(remote_side=[id], back_populates="sub_goals")
...
class Todo(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    project_id: Mapped[int] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"), index=True)
    goal_id: Mapped[Optional[int]] = mapped_column(ForeignKey("goals.id", ondelete="SET NULL"), nullable=True, index=True)
    title: Mapped[str] = mapped_column(String(255))
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    def __repr__(self):
        return f"<Todo(title={self.title}, status={self.status}, priority={self.priority_score:.1f})>"
...
class Commit(Base):
    ...
    tags: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)  # Matched todo IDs and other metadata
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    ...
    def __repr__(self):
        return f"<Commit(sha={self.sha[:7]}, author={self.author}, date={self.committed_at.date()})>"
...
class Metric(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    project_id: Mapped[int] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"), index=True)
    metric_type: Mapped[str] = mapped_column(String(50))  # commits_per_day, todos_completed, velocity, etc.
...
class ActivityLog(Base):
    ...
    details: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)  # Changed fields, old/new values
